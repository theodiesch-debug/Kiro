        ...
        super().__init__(
            ...
            instructions=textwrap.dedent(
                ...
                Du bist Kiro, ein freundlicher, zuverlässiger und intelligenter persönlicher KI-Assistent. Du beantwortest Fragen, erklärst Themen, löst Probleme und führst Aufgaben mit den verfügbaren Werkzeugen aus.
                    ...
                    * Du bist manchmal sarkastisch und sgast offen wenn dir eine Idee nicht gefällt
...
async def my_agent(ctx: JobContext):
    ...
    # Set up a realtime voice AI pipeline using Gemini Live.
    #
    # Gemini Live handles the conversation directly with audio.
    #
    # The pipeline works differently from the traditional
    ...
    #   Camera ─────→ Gemini Live
    #
    # Gemini Live handles speech recognition, reasoning,
    # and speech generation in one realtime model.
    #
    ...
    session = AgentSession(
        ...
        #
        # Gemini Live receives the user's audio and can generate
        # a spoken response directly.
        #
        # Unlike the previous setup, there is no separate
        # Speech-to-Text and Text-to-Speech model.
        #
        # The language is set to German using the BCP-47
        # language code "de-DE".
        #
        # "Enceladus" is the selected Gemini voice.
        #
        # See available realtime models:
    ...
    #
    # This connects the Assistant containing Kiro's instructions
    # to the Gemini Live realtime model.
    await session.start(
        agent=Assistant(),
        room=ctx.room,

        # Configure how audio and video from the LiveKit room
        # are connected to the AgentSession.
        room_options=room_io.RoomOptions(

            # Enable video input so Gemini can receive images
            ...
            #
            # The microphone audio is sent to Gemini Live,
            # which handles the realtime speech processing.
            audio_input=room_io.AudioInputOptions(

                # Use AI Coustics noise cancellation to improve
                # the quality of the microphone audio.
    ...
    #
    # After this point Kiro can receive:
    #
    #   - Microphone audio
    #   - Camera video
    #
    # and can send generated speech back into the room.
    await ctx.connect()
...
if __name__ == "__main__":
    ...
    #
    # "lk agent dev" starts this file in development mode
    # and automatically reloads it when the source code changes.
    cli.run_app(server)